...
class  AddExelFeeding extends React.Component{
    constructor(){
        super()
    this.state={
        arr:{}
    ...
    submitFile=()=>{
        firebase.database().ref().child(this.state.branch).child('feedpoints').set(this.state.arr,(err,doc)=>{
            if(!err){
    ...
    return (
        <div>
            <input className="btn-primary btn-sm" type="file" onChange={this.uploadfile} style={{width: "100%"}} />
